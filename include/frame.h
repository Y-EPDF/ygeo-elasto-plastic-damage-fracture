...
#define INS int
#define DBL double
#define YINT long int
#define CHR char
#define YUINT unsigned int
...
#define INT1NULL ((YINT*)NULL)
#define INT2NULL ((YINT**)NULL)
#define INT3NULL ((YINT***)NULL)
...
/* 2×2 矩阵对称化：msym = 0.5 * (m + m^T) */
#define YSYMMAT2(m, msym) \
{ \
    (msym)[0][0] = (m)[0][0]; \
    (msym)[1][1] = (m)[1][1]; \
    double _s01 = 0.5 * ((m)[0][1] + (m)[1][0]); \
    (msym)[0][1] = _s01; \
    (msym)[1][0] = _s01; \
}
...
{ \
    ...
    _M[1][0] = (dt) * (A)[1][0]; \
    _M[1][1] = (dt) * (A)[1][1]; \
    double _tr = 0.5 * (_M[0][0] + _M[1][1]); \
    double _det = _M[0][0]*_M[1][1] - _M[0][1]*_M[1][0]; \
    double _delta2 = _tr*_tr - _det; \
    double _B[2][2]; \
    _B[0][0] = _M[0][0] - _tr; \
    _B[0][1] = _M[0][1]; \
    _B[1][0] = _M[1][0]; \
    _B[1][1] = _M[1][1] - _tr; \
    double _c0 = exp(_tr); \
    if (fabs(_delta2) < 1e-14) { \
        (E)[0][0] = _c0 * (1.0 + _B[0][0]); \
        (E)[0][1] = _c0 * (_B[0][1]); \
        (E)[1][0] = _c0 * (_B[1][0]); \
        (E)[1][1] = _c0 * (1.0 + _B[1][1]); \
    } else { \
        double _delta = sqrt(fabs(_delta2)); \
        double _ch, _s; \
        if (_delta2 > 0) { \
            _ch = cosh(_delta); \
            _s  = sinh(_delta) / _delta; \
        } else { \
            _ch = cos(_delta); \
            _s  = sin(_delta) / _delta; \
        } \
        (E)[0][0] = _c0 * (_ch + _s * _B[0][0]); \
        (E)[0][1] = _c0 * (_s * _B[0][1]); \
        (E)[1][0] = _c0 * (_s * _B[1][0]); \
        (E)[1][1] = _c0 * (_ch + _s * _B[1][1]); \
    } \
}
...
#if NeedFunctionPrototypes 
  YINT m2,              /* m2 1D arrays size          */
  YINT m1               /* of size   m1 DBL           */ 
#endif
); 
DBL ***TalDBL3(  /* alocate array */
#if NeedFunctionPrototypes 
  YINT m3,              /* m3 2D arrays size m2*m1         */
  YINT m2,              /* m3*m2 1D arrays                 */
  YINT m1               /* of size   m1 DBL                */ 
#endif
);
YINT *TalINT1(  /* alocate array */
#if NeedFunctionPrototypes 
  YINT m1               /* size          */
#endif
); 

YINT **TalINT2(  /* alocate array */
#if NeedFunctionPrototypes 
  YINT m2,              /* m2 1D arrays size          */
  YINT m1               /* of size   m1 YINT           */
#endif
); 
YINT ***TalINT3(  /* alocate array */
#if NeedFunctionPrototypes 
  YINT m3,              /* m3 2D arrays size m2*m1         */
  YINT m2,              /* m3*m2 1D arrays                 */
  YINT m1               /* of size   m1 YINT                */ 
...
YINT Getname(  /* return i for argv[i] that follows after "name" like -i file */
#if NeedFunctionPrototypes 
  YINT argc,          /* number of argument in argv      */
  CHR **argv,        /* arguments                        */
...
#if NeedFunctionPrototypes 
  FILE *fptr,          /* file          */
  YINT iinit,           /* initialise to */
  YINT n1,              /* size          */
  YINT **i1aray          /* array pointer */
#endif
);
...
FILE *TwriteCHR2(  /* write array */
  ...
  CHR *namep,          /* file name                 */
  FILE *fpt,           /* file                      */
  YINT ndigit,          /*number of digits  ignored  */
  YINT nperli,          /* numbers per line          */
  YINT n1,              /* size                      */
...
void TsortDBL1(  /* sort array = smallest ... largest*/
  ...
  YINT *i1y             /* array pointer (array to be rearanged) */
#endif
);
void TsortINT(   /* sort array = smallest ... largest*/
#if NeedFunctionPrototypes 
  YINT n,               /* size                                  */
  YINT nsort,           /* total number of arrays to be sorted   */
  YINT nrear,           /* total number of arrays                */
  YINT **i2             /* array pointers (sorted or rearanged)  */
#endif
); 
...
void readcode(  /* read coded array */
  ...
  YINT *i1num           /* YINT array [0]=ndigit; [1]=nnum
...
void codeINTtoDBL(  /* translate YINT to DBL array */
#if NeedFunctionPrototypes 
...
void codeDBLtoINT(  /* translate DBL to YINT array */
#if NeedFunctionPrototypes 
  DBL *d1num,          /* DBL array [0]=ndigit; [1]=nnum
                          all between -1 and +1                */  
  YINT *i1num           /* YINT array [0]=ndigit; [1]=nnum
