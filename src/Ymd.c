...
static void Yjd2TRIANGLE(  /* mesh triangle  */
	    ...
	    d1elfr, iusefn, mdfnfr, mdfnno, i2dfnn, d1dffr, d1dfpe, d1elpe, d1dfpt, d1elpt,
            i2elnext, i1edfnf,i1getmasterfem,i1notoel, i1nobf0, i2elel0 
            )
  YINT    melem; YINT   mnopo; YINT  nelest; YINT  nnopst;
  YINT    iprop; YINT **i2pmij; YINT mpmrow; YINT nprop;
  YINT *n0elem; YINT  *n0nopo;
  DBL *d1nccx; DBL  *d1nccy; DBL *d1ncix; DBL  *d1nciy; DBL *d1nvcx;
  DBL *d1nvcy; DBL  *d1sdel;
  YINT *i1elpr; YINT  *i1elprtmp; YINT *i1jnef; YINT *i1jnen; YINT *i1next; YINT *i1nobf; YINT *i1nopr; YINT *i1nowe;
  YINT **i2elto;
  DBL *d1elfr; YINT iusefn; YINT mdfnfr; YINT mdfnno; YINT **i2dfnn; DBL *d1dffr;
  DBL *d1elpe; DBL *d1dfpe; DBL *d1elpt; DBL *d1dfpt; 
  YINT **i2elnext; YINT *i1edfnf; YINT *i1getmasterfem; YINT *i1notoel; YINT *i1nobf0;YINT **i2elel0 ;
  
{ YINT nelem, nnopo;
  YINT i,j,in,jn,kn,ijnew,ielem,irow;
  YINT ipropj = -1;
 
  YINT fracture_flag; /* Flag that identifies a joint (i.e. element edge) element that belongs to the DFN */
...
i1nobf0[i2elto[1][ielem]]=1;

i1nobf0[i2elto[0][ielem]]=1;
...
static void Ymd2TRIANGLE(  /* mesh triangle  */
  ...
  YINT    melem; YINT   mnopo; YINT  nelest; YINT  nnopst;
  YINT    ipemb; YINT   iprop;
  YINT *n0elem; YINT  *n0nopo;
  DBL *d1nccx; DBL  *d1nccy; DBL *d1ncix; DBL  *d1nciy; DBL *d1nvcx;
  DBL *d1nvcy; DBL  *d1sdel; YINT  *i1elpr; YINT *i1elprtmp; YINT *i1mnnf; YINT *i1mnnn;
  YINT *i1nobf; YINT *i1nopr; YINT *i1nowe; YINT **i2elto;
{ YINT nelem, nnopo;
  DBL x,y;
  YINT i,j,in,jn,innew,ielem,jelem;
  YINT i1t[9]={3,0,2, 4,1,0, 5,2,1};
  YINT i1new[6];
...
static void Ymdicsreted2TRIANGLE(  /* mesh triangle discrete */
  ...
  YINT *n0elem; YINT  *n0nopo;
  DBL *d1nccx; DBL  *d1nccy; DBL *d1ncix; DBL *d1nciy; DBL  *d1nvcx;
  DBL *d1nvcy; YINT  *i1elpr; YINT *i1nobf; YINT *i1nopr; YINT *i1nowe; YINT **i2elto;
{ YINT nelem, nnopo;
  YINT i,ielem,inopo;
  nelem=(*n0elem);
  nnopo=(*n0nopo);
...
{ YINT nelem, nnopo;
  DBL x,y;
  YINT i,j,k,in,jn,innew,ielem,jelem,kelem;
  YINT i1t[18]={3,0,6,  0,4,6, 4,1,6,  1,5,6,  5,2,6,  2,3,6};
  YINT i1new[7];
...
/*********************PUBLIC********************************************************/
void Ymd(   ydc, yde, ydi, ydn,ydmn, ydpe, ydpn, ydpm, ydfn , ydpj   /***  mesh elements  ***/
        )
...
{ YINT nelest, nnopst;
  ...
  YINT inopo;
  YINT imesh,imestyp;
  YINT nProps;         /* max of number Pr sets to be meshed together in one line of i2pmset  */
  YINT *i1props;       /* List of Property sets to be meshed together                         */
  YINT *i1mnnf;        /* mesh node new first for each old node                               */
  YINT *i1mnnn;        /* mesh node new next for each new node                                */
  YINT *i1jnef;        /* joint node element first for each old node                          */
  YINT *i1jnen;        /* joint node element next for each new joint                          */
  DBL *d1sdel;
  YINT *i1next;        /* stores index of the element next to joint                           */
  YINT *i1isMeshing;   /* indicates if meshing is allowed for the selected Pr Set,
                         only if prtype = YTE2TRIELS || YTE2TRIRIG || YTE2PLANESTRESS ||  
                         YTE2PLANESTRAIN */
  YINT i0, i1, i2, i3;
  
ydmn->mnopo = ydn->mnopo;  // 直接复制 mnopo
  ydmn->nnopo = ydn->nnopo;  // 直接复制 nnopo

  if(ydmn->d2nci==DBL2NULL)
  { ydmn->d2ncc=TalDBL2(2,ydmn->mnopo);
    ydmn->d2nci=TalDBL2(2,ydmn->mnopo);
    for (i=0;i<2;i++)
    { for(j=0; j<ydmn->mnopo; j++)
      { ydmn->d2ncc[i][j]=ydn->d2ncc[i][j];
        ydmn->d2nci[i][j]=ydmn->d2ncc[i][j];
    } 
  ...
  if (yde->i2elel0 == INT2NULL)
  {
      yde->i2elel0 = TalINT2(3, yde->melem);
      for (i = 0; i < 3; i++)
      {
...
if(ydn->i1getmaster == INT1NULL)
  { ydn->i1getmaster=TalINT1(ydn->mnopo);
    ydn->i1getmasterfem=TalINT1(ydn->mnopo);
    ydn->i1nobf0=TalINT1(ydn->mnopo);
    for (i=0;i<ydn->mnopo;i++)
	{ ydn->i1getmaster[i]=-1;
    ydn->i1getmasterfem[i]=-1;
    ydn->i1nobf0[i]=-1;
  
    }
  }
if(ydn->i1notoel == INT1NULL)
  { 
    ydn->i1notoel=TalINT1(ydn->mnopo);
  ...
  if(yde->d2centroid==DBL2NULL)
  { yde->d2centroid=TalDBL2(2,yde->melem);
    for (i=0;i<2;i++)
    { for(j=0; j<yde->melem; j++)
      { yde->d2centroid[i][j]=-9000000.000;
    } }
    ...
    { iprop = ydpm->i2pmset[2][icom];
      ...
      { for(irow=0; irow<ydpm->mpmrow; irow++)
        ...
        { if(i1jnef==INT1NULL)
          ...
          ydn->i1getmasterfem,ydn->i1notoel, ydn->i1nobf0,yde->i2elel0
