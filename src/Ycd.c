...
static void Ycdvel2TRI( /* find maximum squered velocity */
  ...
  YINT   iprop;
  DBL *d0mxv2;
  DBL *d1nvcx; DBL *d1nvcy; YINT *i1elpr; YINT **i2elto;
{ YINT ielem;
...
static void Ycdcor2TRI(  /* find element coordinates */
  ...
  YINT *i1elpr; YINT *i1nobf; YINT **i2elto;
{ YINT ielem,i;
...
static void Ycdprocess(
  ...
  YINT    melem; YINT micoup; YINT  nelem; YINT   nprop;
  DBL   dcstec; DBL diezon; DBL *d0iedi;
  YINT *i0iecff;
  DBL  *d1nccx; DBL  *d1nccy; DBL *d1nccz; DBL *d1nvcx; DBL *d1nvcy;
  DBL  *d1nvcz; YINT  *i1elcf; YINT *i1elpr; YINT *i1iecn; YINT *i1iect;
  YINT  *i1nobf; YINT  *i1ptyp; YINT **i2elto;
{ YINT ncelx,ncely,ncelz;  /* total number of x, y, z cells   */
  YINT nelemd;             /* twice total number of elements  */
  DBL diam;               /* maximum diameter of element     */
  DBL dmxv2;              /* maximum displacement increment  */
  YINT icoup,jcoup;        /* couple                          */
  YINT icouco;             /* couple's contactor              */
  YINT icouta;             /* couple's target                 */
  YINT ielem;                /* element                       */
  YINT ielemx;               /* element assigned to cell x    */
  YINT ielemy;               /* element assigned to cell y    */
  YINT ielemz;               /* element assigned to cell z    */
  YINT ihx,ihy,ihz;          /* x, y, z  head of a   list     */
  YINT iminx,iminy,iminz;    /* space boundaries              */
  YINT imaxx,imaxy,imaxz;    /* space boundaries              */
  YINT iprop;                /* element property id           */
  YINT ix,iy,iz;             /* x, y, z cell                  */
  YINT jelemx;               /* element assigned to cell x    */
  ...
  YINT *i1cnx;      /* contactor next  x                      */
  YINT *i1cny;      /* contactor next  y                      */
  YINT *i1cnz;      /* contactor next  z                      */
  YINT *i1cfz;      /* contactor first z                      */
  YINT *i1eccx;     /* element coordinate current x           */
  YINT *i1eccy;     /* element coordinate current y           */
  YINT *i1eccz;     /* element coordinate current z           */
  YINT  i1heax[5];  /* heads of 5 connected lists for x cells */
  YINT  i1heay[5];  /* heads of 5 connected lists for y cells */
  YINT  i1heaz[2];  /* heads of 2 connected lists for z cells */
  YINT *i2cfx[2];   /* contactor first x                      */
  YINT *i2cfy[2];   /* contactor first y                      */
  ...
  { nelemd=2*nelem;
    ...
    { /* unmark new couples */
      ...
      for(ielem=0;ielem<nelem;ielem++)
      { i1eccx[ielem]=(YINT)(d1eccx[ielem]/diam);
        i1eccy[ielem]=(YINT)(d1eccy[ielem]/diam);
        i1eccz[ielem]=(YINT)(d1eccz[ielem]/diam);
      }
