...
static YINT  i1num[100];    /* numbers for space saving format     */
...
static void Yod2TRISOF(  /* small strain softening triangle output */
  ...
  YINT   icoutp; YINT   iprop;
  DBL  *d1nccx; DBL *d1nccy; DBL *d1ncix; DBL  *d1nciy; DBL *d1nvcx;
  DBL  *d1nvcy; DBL *d1sdel; YINT *i1elpr; YINT **i2elto;
{ DBL voli,volc;
  DBL  B[2][2]; /* left Cauchy-Green strain tensor */
  DBL  D[2][2]; /* rate of deformation (stretching) tensor */
  DBL  E[2][2]; /* strain tensor (small strains) */
...
static void Yod2TRIRIG(  /* rigid triangle output */
  ...
  YINT   icoutp; YINT   iprop;
  DBL  *d1nccx; DBL *d1nccy; DBL *d1nvcx; DBL  *d1nvcy; YINT *i1elpr;
  YINT **i2elto;
{ YINT ielem;
  YINT i;
  for(ielem=0;ielem<nelem;ielem++)
...
static pv_tensor_t* pv_stress_p;
static pv_tensor_t* pv_stress_von_mises;
...
void pv_init(int num_elements, int upper_bound_num_joints, int num_rebars, int upper_bound_num_1D_joints, int num_dead_nodes, int upper_bound_acoustic_events, const char* file_prefix, double realtime_per_timestep)
{
    ...
    pv_stress_p                        = malloc( num_elements * sizeof(pv_tensor_t) );
    if(pv_stress_p == NULL) { exit(1); }
    pv_stress_von_mises                        = malloc( num_elements * sizeof(pv_tensor_t) );
    if(pv_stress_von_mises == NULL) { exit(1); }
...
void pv_write_stress_p(int munjiza_element_id, pv_tensor_t tensor)
{
    int element_id;
    element_id =  translate_munjiza_element_to_sane(munjiza_element_id);

    if(element_id >= pv_num_elements) { printf("Element ID %d out of range", element_id); exit(1); }
    pv_stress_p[element_id] = tensor;
}
void pv_write_stress_von_mises(int munjiza_element_id, pv_tensor_t tensor)
{
    int element_id;
    element_id =  translate_munjiza_element_to_sane(munjiza_element_id);

    if(element_id >= pv_num_elements) { printf("Element ID %d out of range", element_id); exit(1); }
    pv_stress_von_mises[element_id] = tensor;
}
...
//printf("DEBUG: %s", tmp_filename);
   ...
   fprintf(vtp_file, "<CellData Tensors='stress stress_p strain principal_stress' Vectors='principal_stress_direction_1 principal_stress_direction_2' Scalars='property_id'>\n");
   ...
   fprintf(vtp_file, "\n</DataArray>\n");
       ...
       /* Write out stress_p */
   fprintf(vtp_file, "<DataArray type='Float32' Name='stress_p' NumberOfComponents='9' format='ascii'>\n");

   for(i = 0; i < pv_num_elements; ++i)
   {
       fprintf(vtp_file, "%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f ", (double)pv_stress_p[i].a11,  (double)pv_stress_p[i].a12,  (double)pv_stress_p[i].a13,  (double)pv_stress_p[i].a21,  (double)pv_stress_p[i].a22,  (double)pv_stress_p[i].a23,  (double)pv_stress_p[i].a31,  (double)pv_stress_p[i].a32,  (double)pv_stress_p[i].a33);  
   }
   fprintf(vtp_file, "\n</DataArray>\n");


       /* Write out stress_von_mises */
   fprintf(vtp_file, "<DataArray type='Float32' Name='stress_von_mises' NumberOfComponents='9' format='ascii'>\n");

   for(i = 0; i < pv_num_elements; ++i)
   {
       fprintf(vtp_file, "%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f ", (double)pv_stress_von_mises[i].a11,  (double)pv_stress_von_mises[i].a12,  (double)pv_stress_von_mises[i].a13,  (double)pv_stress_von_mises[i].a21,  (double)pv_stress_von_mises[i].a22,  (double)pv_stress_von_mises[i].a23,  (double)pv_stress_von_mises[i].a31,  (double)pv_stress_von_mises[i].a32,  (double)pv_stress_von_mises[i].a33);  
   }
   fprintf(vtp_file, "\n</DataArray>\n");
...
{ YDC ydc=&(yd->ydc);
  ...
  YINT iprop, jprop, ihys, i, inode, nres;
  CHR namef[300];
  CHR cindex[50];
  static YINT ncall=0;
  static YINT ndefinitions=0;
  static YINT ncallBRK=0;
  static YINT ncallSFT=0;
  static YINT ncallHYFR=0;
  static YINT ncallFRAC=0;
  static YINT ires=0;    //Counter for the restart data files
  FILE *fout=FILENULL;
  DBL tmp;
  static YINT *nodes;
  ...
  pv_tensor_t element_stress_p;
 pv_tensor_t element_stress_von_mises;
  ...
  //if(((ydc->ncstep%output_frequency)==0)||(ydc->ncstep>4700&&ydc->ncstep<4799))
    ...
    { if((ydpe->i1ptyp[iprop])==(YTE2TRIELS) ||
      ...
      { for(ielem=0;ielem<yde->nelem;ielem++)
	    ...
	    { if (ydpe->i1ptyp[iprop] == YTE2TRIELS)
            ...
            */
            element_stress.a11=yde->sigma00[0][ielem];
            element_stress.a12=yde->sigma10[0][ielem];
            element_stress.a13=0.0;
            element_stress.a21=yde->sigma10[0][ielem];
            element_stress.a22=yde->sigma11[0][ielem];
            element_stress.a23=0.0;
            element_stress.a31=0.0;
            element_stress.a32=0.0;
            element_stress.a33=yde->sigma33[0][ielem];
	    pv_write_stress(ielem, element_stress);


// 计算Von Mises应力（平面应变工况，原文二维Cook膜采用此工况）
double sigma_xx = element_stress.a11;
double sigma_yy = element_stress.a22;
double tau_xy = element_stress.a12;
double sigma_zz = element_stress.a33; // 已经由本构算好
 // 3D Von Mises 
 double term1 = (sigma_xx - sigma_yy) * (sigma_xx - sigma_yy); 
 double term2 = (sigma_yy - sigma_zz) * (sigma_yy - sigma_zz); 
 double term3 = (sigma_zz - sigma_xx) * (sigma_zz - sigma_xx);
// 平面应变Von Mises应力公式：σ_VM = sqrt((σxx - σyy)² + 3τxy²)
double von_mises_stress = sqrt(pow(sigma_xx - sigma_yy, 2) + 3 * pow(tau_xy, 2));
double von_mises_stress3D = sqrt(0.5 * (term1 + term2 + term3) + 3.0 * tau_xy * tau_xy);
            element_stress_von_mises.a11=von_mises_stress;
            element_stress_von_mises.a12=0.0;
            element_stress_von_mises.a13=0.0;
            element_stress_von_mises.a21=0.0;
            element_stress_von_mises.a22=von_mises_stress3D;
           element_stress_von_mises.a23=0.0;
            element_stress_von_mises.a31=0.0;
            element_stress_von_mises.a32=0.0;
            element_stress_von_mises.a33=von_mises_stress;




// 可选：输出或存储Von Mises应力（示例：假设pv_write_von_mises为自定义输出函数）
pv_write_stress_von_mises(ielem, element_stress_von_mises);
            ...
            element_stress_p.a11=(yde->sigma00[0][ielem]+yde->sigma11[0][ielem]+yde->sigma33[0][ielem])/3.0;
            element_stress_p.a12=0.0;
            element_stress_p.a13=0.0;
            element_stress_p.a21=0.0;
            element_stress_p.a22=-(yde->sigma00[0][ielem]+yde->sigma11[0][ielem]+yde->sigma33[0][ielem])/3.0;
            element_stress_p.a23=0.0;
            element_stress_p.a31=0.0;
            element_stress_p.a32=0.0;
            element_stress_p.a33=-(yde->sigma00[0][ielem]+yde->sigma11[0][ielem])/2.0;
	    pv_write_stress_p(ielem, element_stress_p);





            	 T[0][0] = yde->sigma00[0][ielem];
                 T[1][1] =yde->sigma11[0][ielem];
                 T[0][1] = yde->sigma10[0][ielem];
                 T[1][0] = yde->sigma10[0][ielem];
    ...
    { if((ydpj->i1ptyp[jprop])==(YTE2JOINTS))
	  ...
	  if((ipropc<0)&&((ipropc+YIPROPMAX)==(jprop+ydpe->nprop))) //! Broken joint output
	    ...
	    //! If joint belongs to an excavation boundary, set sliding, opening, area and length to zero.
            if((yde->i2elnext[0][ielem]!=-1)&&(yde->i2elnext[1][ielem]!=-1))
            { 
