...
{ CHR *namep;
 ...
 else if(CHRcmp(namep,"IUSEECZM",8)==0)
  { INTr(finp,&(ydc->iuseeczm));
  }
...
yde->d2elR     = DBL2NULL;
yde->d2elV     = DBL2NULL;
yde->d2elAlpha = DBL2NULL;
yde->d2elState = DBL2NULL;
...
/* --- 新增：乘法分解状态量 --- */
yde->sigma00    = DBL2NULL;
yde->sigma10    = DBL2NULL;
yde->sigma11    = DBL2NULL;
yde->sigma33    = DBL2NULL;
yde->sigma31    = DBL2NULL;
yde->sigma32    = DBL2NULL;
yde->ss00    = DBL2NULL;
yde->ss01    = DBL2NULL;
yde->ss10    = DBL2NULL;
yde->ss11    = DBL2NULL;
...
yde->d2D=DBL2NULL;
 yde->i1elfr =INT1NULL;
  yde->i2elel0 = INT2NULL;

 yde->d1J=DBL1NULL; 
 yde->d1J2=DBL1NULL; 
  yde->d1volce=DBL1NULL; 
  ...
  /* ECZM */
  ydn->i1nobf0=INT1NULL;
  ydn->i1getmaster=INT1NULL; 
  ydn->i1getmasterfem=INT1NULL; 
  ydn->i1notoel=INT1NULL; 
  ydn->i1remeshf =INT1NULL;

  ydmn->mnopo=0;  ydmn->nnopo=0;
  ydmn->d2ncc=DBL2NULL;
  ydmn->d2nci=DBL2NULL;
  ydmn->i2elsort=INT2NULL;
  ydmn->i1elcount=INT1NULL;
  ydmn->i2elno=INT2NULL;
  ydmn->i2elnext=INT2NULL;
  ydmn->i1head=INT1NULL;
  ydmn->i1boundaryflag=INT1NULL;
  //体积锁定
  ydmn->d1noa0=DBL1NULL;
  ydmn->d1noac=DBL1NULL;
  ...
  ydpe->i1psup=INT1NULL;
    /* ---------------- 显式塑性 + 扩展次载面模型材料参数（DP版）初始化 ---------------- */


/* Drucker–Prager 屈服参数（有限变形） */
ydpe->d1phi   = DBL1NULL;  /* 摩擦角 φ（使用度制） */
ydpe->d1DPc     = DBL1NULL;  /* 内聚力 c */
ydpe->d1sig_t = DBL1NULL;  /* 抗拉截断 σ_t（<=0 表示不启用） */

/* 等向硬化参数（F(H)） */
ydpe->d1F0    = DBL1NULL;  /* 初始屈服函数值 */
ydpe->d1h1    = DBL1NULL;  /* 非线性硬化幅度 */
ydpe->d1h2    = DBL1NULL;  /* 非线性硬化速率 */
ydpe->d1Hc    = DBL1NULL;  /* 线性硬化模量（0 表示不用） */

/* 动硬化能量参数（可选，Mk） */
ydpe->d1Ck    = DBL1NULL;  /* ψk 系数 */

/* 弹性核能量参数（Mc） */
ydpe->d1Cc    = DBL1NULL;  /* ψc 系数 */

/* 动硬化演化与极限（可选） */
ydpe->d1bk    = DBL1NULL;  /* 动硬化饱和值 */
//ydpe->d1ck    = DBL1NULL;  /* 动硬化速率参数 */
//ydpe->d1nk    = DBL1NULL;  /* 动硬化自旋系数 */

/* 次载面参数（R, Rc 演化） */
ydpe->d1URu     = DBL1NULL;  /* U(R) 放大系数 */
ydpe->d1uc    = DBL1NULL;  /* 重装载放大系数 */
ydpe->d1Re    = DBL1NULL;  /* 准弹性阈值 R_e */
ydpe->d1Chi   = DBL1NULL;  /* 弹性核极限比 χ */

/* 塑性自旋参数（式(113)） */
ydpe->d1nP    = DBL1NULL;  /* 塑性自旋系数 n^p */
ydpe->d1n2    = DBL1NULL;  /* 弹性核自旋系数 n^c */
ydpe->d1nn    = DBL1NULL;  /* 塑性自旋系数 n^p */
/* 算法选项 */
ydpe->i1use_ext_subloading = INT1NULL; /* 1=扩展次载面（Mc），0=初始次载面 */


ydpe->i1use_kinhard        = INT1NULL; /* 1=启用 Mk/ψk 与 Fpkd 更新；0=关闭 */
...
{ CHR *namep;
  ...
  { TformINT1(finp,-1,yde->melem,&(yde->i1elpr));
    ...
    TformINT1(finp,-1,yde->melem,&(yde->i1elfr));
  }
...
/* Drucker–Prager 屈服（有限变形） */
else if (CHRcmp(namep,"D1PHI",5)==0)   /* 摩擦角 φ（度制） */
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1phi)); }
else if (CHRcmp(namep,"D1DPC",5)==0)     /* 内聚力 c */
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1DPc)); }
else if (CHRcmp(namep,"D1SIG_T",7)==0) /* 抗拉截断 σ_t（<=0 不启用） */
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1sig_t)); }

/* 等向硬化 F(H) */
else if (CHRcmp(namep,"D1F0",4)==0)
{ 
  TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1F0));
 }
else if (CHRcmp(namep,"D1H1",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1h1)); }
else if (CHRcmp(namep,"D1H2",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1h2)); }
else if (CHRcmp(namep,"D1HC",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1Hc)); }

/* 动硬化/弹性核能量参数 */
else if (CHRcmp(namep,"D1CK",4)==0)   /* ψk */
{ 
  TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1Ck)); 
}
else if (CHRcmp(namep,"D1CC",4)==0)   /* ψc */
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1Cc)); }

/* 动硬化演化（可选） */
else if (CHRcmp(namep,"D1BK",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1bk)); }
//else if (CHRcmp(namep,"D1CKP",5)==0)  /* 速率参数，可用其他关键字 */
//{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1ck)); }
//else if (CHRcmp(namep,"D1NK",4)==0)
//{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1nk)); }

/* 次载面与 Masing 扩展 */
else if (CHRcmp(namep,"D1URU",5)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1URu)); }
else if (CHRcmp(namep,"D1UC",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1uc)); }
else if (CHRcmp(namep,"D1RE",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1Re)); }
else if (CHRcmp(namep,"D1CHI",5)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1Chi)); }

/* 自旋参数 */
else if (CHRcmp(namep,"D1NP",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1nP)); }
else if (CHRcmp(namep,"D1N2",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1n2)); }
else if (CHRcmp(namep,"D1NN",4)==0)
{ TformDBL1(finp,R0,ydpe->mprop,&(ydpe->d1nn)); }
/* 算法选项 */
else if (CHRcmp(namep,"I1USE_EXT_SUBLOADING",20)==0)
{ TformINT1(finp,-1,ydpe->mprop,&(ydpe->i1use_ext_subloading)); }


else if (CHRcmp(namep,"I1USE_KINHARD",13)==0)
{ TformINT1(finp,-1,ydpe->mprop,&(ydpe->i1use_kinhard)); }
