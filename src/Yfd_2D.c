...
double* SIG,double* AL, double* I_M, double* De_m,double* C_m,double*Ep_M
,double H,double F,double Hdh
...
)
{
    ...
    /* 临时量 */
    double Pivot, Factor, SumVal,R_eff,R,UR,DPX;
    double RHS_Vec[6],Dex_M[i];
    double Mat_K[6][6];
    ...
    double Da_M[6];
    double DP_M[6];
double    fhn = sqrt(2.0 / 3.0);
double    Fdh = F0 * S_r * CH;
    YINT CLU = 1.0;
...
L10:
    ...
    DPX = sqrt(DPX);
    {
        double DR = UR * DPX; /* DR 目前没被用到，先保留计算 */
        (void)DR;
    }
...
wvec[0] = 0.5 * (W3D[2][1] - W3D[1][2]);  /* wx */
wvec[1] = 0.5 * (W3D[0][2] - W3D[2][0]);  /* wy */
wvec[2] = 0.5 * (W3D[1][0] - W3D[0][1]);  /* wz */
...
/* -------------------------------------------------------------------------
   ...
   物理含义：
     z 是 F&T 框架中由 “伸长张量 V” 与 “变形率 D” 的耦合产生的旋转分量，
     它反映了材料在非刚体旋转下，由主伸长方向变化引起的附加旋转。

   数学化简：
     原式可写为  z = axial( D*V − (D*V)^T ) / 2
     即：
       1) 先计算 A = D·V
       2) 再取其反对称部分 B = A − A^T
       3) 最后 z = 1/2 * axial(B)
     axial(B) 将 3×3 反对称张量 B 映射为向量 (B32−B23, B13−B31, B21−B12)

   plane strain 特性：
     在平面应变下，D3D 与 Vloc 的 out‑of‑plane 分量大多为零，
     因此 zvec 只有 z 分量通常非零（对应平面内的附加旋转）。

   结果：
     zvec[] 为 F&T 中用于构造物质旋转 ω 的关键项： ω = w + z + ...
   ------------------------------------------------------------------------- */
...
double SIG[6];  // 对应你算法中的应变增量输入
SIG[0] = sigma00[ielem];          // εxx
SIG[1] = sigma11[ielem];          // εyy
SIG[2] = sigma33[ielem];          // εzz（平面应变下为0）
SIG[3] = sigma10[ielem];    // γxy = 2εxy（工程剪应变定义）
SIG[4] = sigma31[ielem];    // γyz = 2εyz（平面应变下为0）
SIG[5] = sigma32[ielem];    // γzx = 2εzx（平面应变下为0）
double Ss[6];  // 对应你算法中的应变增量输入
Ss[0] = ss00[ielem];          // εxx
Ss[1] = ss11[ielem];          // εyy
Ss[2] = ss33[ielem];          // εzz（平面应变下为0）
Ss[3] = ss10[ielem];    // γxy = 2εxy（工程剪应变定义）
Ss[4] = ss31[ielem];    // γyz = 2εyz（平面应变下为0）
Ss[5] = ss32[ielem];    // γzx = 2εzx（平面应变下为0）
double EP_M[6];  // 对应你算法中的应变增量输入
EP_M[0] = M00[ielem];          // εxx
EP_M[1] = M11[ielem];          // εyy
EP_M[2] = M33[ielem];          // εzz（平面应变下为0）
EP_M[3] = M10[ielem];    // γxy = 2εxy（工程剪应变定义）
EP_M[4] = M31[ielem];    // γyz = 2εyz（平面应变下为0）
EP_M[5] = M32[ielem];    // γzx = 2εzx（平面应变下为0）
int I_m[6] = {1, 1,1, 0, 0, 0}; 
// ---------------- 适配步骤：设置应变控制逻辑（贴合你算法的C_m变量） ----------------
// 你的算法用C_m控制“已知应变”（1=未知，0=已知），平面应变下仅x-y方向有变形：
int C_m[6] = {1, 1, 0, 1, 0, 0};  // z方向应变固定为0，剪应变yz/zx固定为0
// （若你有自定义控制逻辑，可保留，仅需确保已知应变对应非旋转构型）
...
calc_exsub(
 ...
 SIG,Ss, I_m,De_m,C_m,EP_M,
H[ielem], R[ielem], Rc[ielem]
...
sigma_nr[1][1] = SIG[1];
sigma_nr[2][2] = SIG[2];
sigma_nr[0][1] = sigma_nr[1][0] = SIG[3] / 2.0;  // 工程剪应力→张量剪应力（γxy=2εxy）
sigma_nr[1][2] = sigma_nr[2][1] = SIG[4] / 2.0;
sigma_nr[0][2] = sigma_nr[2][0] = SIG[5] / 2.0;
...
DBL eta = dpero;  // 阻尼系数（对应你函数参数中的dpero，与原FDEM一致）
...
// for(i=0;i<3;i++)
//       { 
//         assert(d1noac[i1getmaster[(i2elto[i][ielem])]]>0&&d1noa0[i1getmaster[(i2elto[i][ielem])]]>1e-9);
//         jbarno[i]=d1noac[i1getmaster[(i2elto[i][ielem])]]/d1noa0[i1getmaster[(i2elto[i][ielem])]];
//       if(i1nobf0[i2elto[i][ielem]]==1){ nobf=nobf+1; }
//     }
...
//  // double vol_corr = sqrt(Jbar / JJ);  // 平面应变修正因子（1/2次方）
