...
#include "Yproto.h"
/**********************SOLVERS*****************************************/
static void Ysd2MEC_ECZM(  nnopo,  /* mechanical solver for 2D nodes with x,y d.o.f.  */
                      ...
                      d1pnxy,d1pnyx,d1pnyy,i1nopr,
											i1pnfx,i1pnfy,i1getmaster,ncstep,mcstep
                    )
  YINT   nnopo;
  DBL  dcstec;  DBL  dcgray;  DBL  dctime;  YINT   npnfact;  YINT ibc;
  DBL ***d3pnfac;
  DBL *d1nccx;  DBL *d1nccy;  DBL *d1nfcx;  DBL *d1nfcy;  DBL *d1nmct;
  DBL *d1nvcx;  DBL *d1nvcy;  DBL *d1pnax;  DBL *d1pnay;  DBL *d1pnxx;
  DBL *d1pnxy;  DBL *d1pnyx;  DBL *d1pnyy;  YINT *i1nopr;
	YINT *i1pnfx;	YINT *i1pnfy;YINT *i1getmaster; YINT ncstep; YINT mcstep;
{ YINT inopo,i;
  DBL T[2][2];
  DBL Tinv[2][2];
...
//  /*
if (ncstep <= (0.1*mcstep)) {
    // 前10%步数：切向载荷从0 线性递增到 输入D1PNAT满值
    aX = aX * ncstep / (0.1*mcstep);
    aY = aY * ncstep / (0.1*mcstep); 
...
//  */
  ...
  { if((i1nopr[inopo]==ibc)&&(d1nmct[inopo]>EPSILON)&&(i1getmaster[inopo]==inopo))
...
static void Ysd2TRIRIG( /* mechanical solver rigid triangles  */
  ...
  YINT   nelem;
  DBL  dcstec; DBL   dpero; YINT   iprop;
  DBL *d1nccx; DBL *d1nccy; DBL *d1nfcx; DBL  *d1nfcy; DBL *d1nmct;
...
/********* PUBLIC ********************/
void Ysd(ydc,yde, ydn,ydmn, ydo, ydpe, ydpn   /*** explicit solver of equations   ***/
         )
  YDC ydc; YDE yde; YDN ydn;YDMN ydmn; YDO ydo; YDPE ydpe; YDPN ydpn;
{ DBL Ek,stprev;
  YINT iprop,i,ihys, ibc, ino;

if(ydc->iuseeczm==1)

{ 
for(ino=0;ino<ydn->nnopo;ino++) 
  { if (ydn->i1getmaster[ino]>0&&ydn->i1getmaster[ino]!=ino) {
   ydn->d1nmct[ydn->i1getmaster[ino]]=ydn->d1nmct[ydn->i1getmaster[ino]]+ydn->d1nmct[ino];
for(i=0;i<ydn->nnodim;i++)    
      { ydn->d2nfc[i][ydn->i1getmaster[ino]]=ydn->d2nfc[i][ydn->i1getmaster[ino]]+ ydn->d2nfc[i][ino]  ;
      }
    }
  ...
  { 
    ...
    Ysd2MEC_ECZM(/* mechanical solver for 2D nodes with x,y d.o.f.  */
    ydn->nnopo,
    ...
    ydpn->i1pnfx, ydpn->i1pnfy,ydn->i1getmaster,ydc->ncstep,ydc->mcstep
    );
    }
...
for(i=0;i<ydn->nnodim;i++)    
      { 
        ydn->d2nfc[i][ino]=ydn->d2nfc[i][ydn->i1getmaster[ino]] ;
        ydn->d2nvc[i][ino]=ydn->d2nvc[i][ydn->i1getmaster[ino]] ;
        ydn->d2ncc[i][ino]=ydn->d2ncc[i][ydn->i1getmaster[ino]] ;
      }
    }
  ...
  { if(ydo->i1ohyt[ihys]==(YFLEK))
    { Ek=R0;
     ...
     if(ydc->ncstep%1000==0)
      { printf("R:%.20f,dengxiaosuxingbianxing:%.10f\n", yde->R[0][0], yde->EPS[0][0]);
  }
